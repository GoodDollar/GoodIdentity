...
export default class IDDaoTEMP {
  ...
  constructor(pkey: string) {
    ...
    this.netword_id = 42 // TODO: change to read from settings
  ...
  */
  async register(proposal:Proposal, feeAmount:number): Promise<[typeof Web3PromieEvent]> {
    ...
    let proposalPromise = new Promise((resolve, reject) => {
      this.ipfs.addJSON(proposalJ, (err, result) => {
        if (err) reject(err)
        else resolve(result)
      })
    }
    ...
    let ipfsByte32 = await proposalPromise.then(hash => this.getBytes32FromIpfsHash(hash))
  ...
  // TODO: use https://github.com/daostack/infra/blob/master/contracts/votingMachines/GenesisProtocol.sol#L61
...
// We can try load all pending proposal ids from DAOStack cache  and then use each id:
// Avatar is the Ethereum address of the IdentityDAO
//     const dao = arc.dao(Avatar.toLowerCase())
//const proposals = dao.proposals()

//this.identityContract.proposals[proposalId]..
